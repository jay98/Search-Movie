...
class FetchTrendingMoviesUseCaseTest {
    ...
    private lateinit var fetchTrendingMoviesUseCase: FetchTendingMoviesUseCase

    @Before
    fun setUp() {
        MockitoAnnotations.openMocks(this)
        fetchTrendingMoviesUseCase = FetchTendingMoviesUseCase(movieRepository, configManager)
    }

    @Test
    fun `invoke should return trending movies with config applied`() = runTest {
        // Given
        ...
        `when`(movieRepository.getTrendingMovies()).thenReturn(flowOf(pagingData))

        val result: Flow<PagingData<Movie>> = fetchTrendingMoviesUseCase()
